    ...
    <AnimatePresence>
      {isOpen && photo && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="fixed inset-0 z-[200] bg-black/95 flex"
          onClick={onClose}
        >
          {/* Close button */}
          <button
            onClick={onClose}
            className="absolute top-6 right-6 z-10 text-text-muted hover:text-text-primary text-2xl w-10 h-10 flex items-center justify-center"
          >
            ✕
          </button>
          ...
          {/* Detail panel — full height, no overlap */}
          <motion.div
            initial={{ x: 320 }}
            animate={{ x: 0 }}
            exit={{ x: 320 }}
            className="hidden md:flex md:flex-col w-80 bg-bg-primary border-l border-border overflow-y-auto"
